...
update_configuration <- function(configuration, trait, genoprobs, effects, residual_variance){
...
output <- list()
output[[1]] <- janninkwu_mcmc_one_iteration(configuration = config, 
...
for (i in 2:1000){
  output[[i]]<- janninkwu_mcmc_one_iteration(configuration = output[[i - 1]]$configuration, 
                                             genoprobs = geno$`1`[ , , 100], 
                                             effects = output[[i - 1]]$effects, 
                                             trait = tr, 
                                             residual_variance = output[[i - 1]]$residual_variance,
                                             effects_variance = output[[i - 1]]$effects_variance
                                             )
}
```
...
residual_variance <- 1
effects_variance <- var(effects)
tr <- reducedscan::sim1(aprobs = gp, allelic_series = config2, allelic_effects = effects)
...
for (i in 1:1000){
  ...
  effects <- out$out[[1]] 
  configuration <- out$out[[2]]
  alpha_allelic_number_update <- out$alpha
  bernoulli_allelic_number_update <- out$bernoulli
    # update configuration for fixed allelic number
  configuration <- update_configuration(configuration = configuration, 
                                        trait = tr, 
                                        genoprobs = gp, 
  ...
  for (k in seq_along(effects)){
    effects <- update_effect(effects = effects, 
                              ...
                              trait = tr, 
                              residual_variance = residual_variance,
                              collapsed_genotypes = gp %*% configuration
                              )$out
  }
  ##
  residual_variance <- update_residual_variance(residual_variance = residual_variance, 
  ...
  outs[[i]] <- list(configuration = configuration, 
              effects = effects, 
              effects_variance = effects_variance, 
              residual_variance = residual_variance, 
       alpha = alpha_allelic_number_update, 
       bernoulli = bernoulli_allelic_number_update
       )
}
```
