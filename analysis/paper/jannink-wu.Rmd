...
propose_wider_configuration <- function(current_configuration, group_indicators, allele_to_split){
  ...
  verify_configuration(configuration_proposal)
  return(configuration_proposal)
}     
  
partition_into_two_groups <- function(vector){
...
calc_prob_allelic_number_increase <- function(configuration,
    ...
    shared_alleles <- which(colSums(configuration) > 1) # numeric vector of length up to 4 (for DO)
    allele_to_split <- sample(x = shared_alleles, size = 1)
    founders_to_split <- which(configuration[ , allele_to_split] == 1)
    group_indicators <- partition_into_two_groups(founders_to_split)
    #group_indicators <- rbinom(n = length(founders_to_split), size = 1, prob = 0.5) # binary vector partitioning founders to split -- THIS IS PROBLEMATIC WHEN binomial rv's come up all 1s or all 0s!!
    configuration_proposal <- propose_wider_configuration(configuration, group_indicators, allele_to_split)
...
verify_configuration <- function(configuration){
  ...
  stopifnot(prod(colSums(configuration)) > 0)
}
```
...
config[, 2] <- 1 - config[, 1]
tr <- reducedscan::sim1(aprobs = geno$`1`[ , , 100], allelic_series = config, allelic_effects = c(10, 30))
```
...
```{r}
config2 <- cbind(config, rep(0, 8))
config2[1, 2] <- 0; config2[1, 3] <- 1
outs <- list()
current <- list(configuration = config2, 
                effects = c(20, 20, 1), 
                residual_variance = 2
                )
all_outs <- list()
