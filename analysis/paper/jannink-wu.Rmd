...
library(magrittr)
ro <- outs %>%
  sapply(FUN = function(x)length(x$effects)) %>% 
  rle()
tibble::tibble(ro$lengths, ro$values)
```
...
```{r check_variance_updates}
outs <- list()
current <- list(configuration = config, 
                effects = c(-30, 30), 
                residual_variance = 20
                )
all_outs <- list()
for (i in 1:10000){
  if (i > 1) current <- outs[[i - 1]] # ok since this is the first step in each iteration.
...
sapply(FUN = function(x)x$residual_variance, X = outs) -> rv
rv %>%
  (function(x)x[-c(1:5000)]) %>%
  (function(x) x[40 * 1:125]) %>%
  hist()
rv %>%
  (function(x)x[-c(1:5000)]) %>%
  (function(x) x[40 * 1:125]) %>%
...
```{r}
outs <- list()
tr <- reducedscan::sim1(aprobs = geno$`1`[ , , 100], 
                        allelic_series = config,
                        allelic_effects = c(-30, 30),
                        error_variance = 1
)
current <- list(configuration = 1 - config, 
                effects = c(20, 40), 
                residual_variance = 2
                )
all_outs <- list()
...
```{r}
outs[[300]] -> c2 # use outs from above
current <- c2
outs <- list()
all_outs <- list()
set.seed(2020-01-27)
