...
update_effects_variance <- function(effects_variance, 
  ...
  # calc numerator
  numerator <- dnorm(rep(effects, each = exponent), 
                     mean = 0, 
                     sd = sqrt(proposal)
                     ) * (1 / min(2 * distance, distance + proposal))
  denominator <- dnorm(effects, 
                       mean = 0, 
...
update_allelic_number <- function(configuration, genoprobs, effects, trait){
  ...
  if (increase_indicator){
    cp_out <- calc_prob_allelic_number_increase()
  } else { # propose a decrease in allelic number
    cp_out <- calc_prob_allelic_number_decrease()
  }
...
##
calc_prob_allelic_number_decrease <- function(configuration, alleles_to_combine, genoprobs, effects){
...
##
calc_configuration_combining_alleles <- function(configuration, alleles_to_combine){
...
##
calc_prob_allelic_number_increase <- function(configuration, shared_alleles, genoprobs, effects, effects_variance){
    ...
    new_effect <- rnorm(n = 1, mean = 0, sd = sqrt(effects_variance)) # check this!!
